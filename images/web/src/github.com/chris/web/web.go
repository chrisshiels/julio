...
func lookupendpoint(endpoint string) (addr string, err error) {
...
func getendpoint(endpoint string,
                 port int,
                 path string) (bytes []byte, err error) {
    ...
    resp, err := http.Get(fmt.Sprintf("http://%s:%d/%s", addr, port, path))
    if err != nil {
        return nil, err
    }
...
func getdate(endpoint string) (date string,
    ...
    bytes, err := getendpoint(endpoint, 7001, "/date")
    if err != nil {
        return "", "", "", err
    }
...
func gettime(endpoint string) (time string,
    ...
    bytes, err := getendpoint(endpoint, 7002, "/time")
    if err != nil {
        return "", "", "", err
    }
